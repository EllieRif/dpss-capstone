...
avg_dec <- retail_change %>% group_by(State) %>% summarise(state_avg_change = mean(mask_change, na.rm = TRUE)) %>% mutate(dec = ntile(state_avg_change, 10))

retail_change <- left_join(retail_change, avg_dec, by = "State")

summary(retail_change$mask_change)

biggest_jump <- retail_change %>% filter(dec==10)

biggest_drop <- retail_change %>% filter(dec == 1)

middle_moves <- retail_change %>% filter(dec == 5| dec==6)
