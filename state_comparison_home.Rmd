...
```{r}
res_long_state_results <- state_results %>% pivot_longer(cols = c(coeff_Alabama:pval_Wyoming), names_to = c("est_pval", "state"), names_sep = "_")

res_wide_state_results <- long_state_results %>% pivot_wider(names_from = term, values_from = value)

res_state_coeffs <- wide_state_results %>% filter(est_pval=="coeff")
```
...
res_state_coeffs <- left_join(rr_state_coeffs, statename)

res_state_coeffs <- rr_state_coeffs %>% rename(intercept = `(Intercept)`)

res_state_geo_coeffs <- right_join(census_spatial, state_coeffs, by = c("GEOID"="sfips"))
```
